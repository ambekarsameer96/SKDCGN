...
class CMPDisLoss(nn.Module):
    ...
    def forward(self, real_list, fake_list):
        ...
        for i in range(1, len(real_list), 2): # compare actvation values of each layer
            ...
            # #add KL loss
            # loss += self.weights[j] * self.kl_loss(F.log_softmax(fake_list[i], dim=1), F.softmax(real_list[i], dim=1))
            #add KL loss
            # loss_kl+= self.kl_loss(F.log_softmax(fake_list[i] / self.temperature, dim=1),
            #                 F.softmax(real_list[i] / self.temperature, dim=1))
            j += 1

        loss_kl = self.kl_loss(F.log_softmax(real_list[i]/self.temperature, dim=1), F.softmax(fake_list[i]/self.temperature, dim=1))
