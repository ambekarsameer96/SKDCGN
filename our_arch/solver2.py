	...
	def train(self):
		...
		for epoch in tqdm(range(self.resume_epoch, self.num_epoch)):
			for i, (x_real, noise, label) in enumerate(tqdm(self.train_loader)):
				...
				#add cross entropy loss between dis_real and labels
				bsize_d = dis_real.size() 
				#bsize_d = bsize_d.unsqueeze(-1) 
				
				fake_labels = torch.zeros_like(dis_fake.squeeze())
				#fake_labels = Variable(fake_labels)

				
				d_bce = self.bce( fake_labels, dis_fake.squeeze(),) 
...
#
				...
				# dis_real2, _ = self.D(x_real2, label2) #no want
				# x_fake2 = self.G(noise2, label2).detach() #no want
				# dis_fake2, _ = self.D(x_fake2, label2) #no want
				# d_loss_real2, d_loss_fake2 = self.dis_hinge(dis_real2, dis_fake2) #no want

				# Backward and optimize.
				d_loss = d_loss_real + d_loss_fake + d_bce
						 # + 0.2*(d_loss_real2 + d_loss_fake2)
				...
				#add bce loss 
				bsize_g = dis_fake.size() 
				
				real_labels = torch.ones_like(dis_fake.squeeze())
				#real_labels = real_labels.unsqueeze(-1) 
				
				
				g_bce = self.bce( real_labels, dis_fake.squeeze())
				g_loss = g_loss_feat + g_bce +  lambda_l1 * g_loss_pix
				loss['G/loss_ft'] = g_loss_feat.item()
				loss['G/loss_l1'] = g_loss_pix.item()
				loss['G/loss_bce'] = g_bce.item()
...
#					 noise2 = torch.randn(self.batch_size, self.z_dim).to(self.device)
				...
				if (i+1) % self.log_step == 0:
					...
					print(log)
					#print d_bce and g_bce loss 
					print(g_bce.item()), print(d_bce.item())
...
# 		self.images_holder = tf.compat.v1.placeholder(tf.float32, [None, 128, 128, 3])
# 		self.activations = inception_activations(self.images_holder)
# 		self.real_acts = tf.compat.v1.placeholder(tf.float32, [None, ACTIVATION_DIM], name = 'real_activations')
# 		self.fake_acts = tf.compat.v1.placeholder(tf.float32, [None, ACTIVATION_DIM], name = 'fake_activations')
