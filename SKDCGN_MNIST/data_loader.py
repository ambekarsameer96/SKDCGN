...
class My_Dataset(data.Dataset):
    ...
    def preprocess(self, num_test=100):
...
#        selec_labels =  set(np.load('bad_ids_dw.npy'))  #set(np.load('hier_label.npy'))
        for img_dir, z_path, label_path in zip(self.image_dir.split('+'), self.z_path.split('+'), self.label_path.split('+')):
            ...
            #print('----------***************** Noise shape', noises.shape)
            #print('Size of noise:', noises.shape)
            labels = []
            
            
            for i, fn in enumerate(filenames):
                #if 'fg' not in fn: continue
                arr = fn.split('_')
                #print(arr)
                cls, num = int(arr[0]), int(arr[1])
                idx = cls*10 + num
                labels.append(cls)
#                if labels[idx] not in selec_labels: continue
                #noises = np.random.rand(64)
                #noise_value = noises[idx]
                noise_value = noises[idx][1:].astype(float)
                #self.dataset.append([os.path.join(img_dir, fn), noises[idx], cls])
                self.dataset.append([os.path.join(img_dir, fn), noise_value, cls])
...
class Interpolate_Dataset(data.Dataset):
    ...
    def preprocess(self, num_test=5000):
        ...
        for img_dir, z_path, label_path in zip(self.image_dir.split('+'), self.z_path.split('+'), self.label_path.split('+')):
            ...
            #generate random noise 10000,64 dimension 
            noises = np.random.rand(1000,64)
            #print('----------***************** Noise shape', noises.shape)
...
#            T.Resize(image_size),                         
    ...
    if isReal:
        ...
        print('I am not using Real image dataloader')
