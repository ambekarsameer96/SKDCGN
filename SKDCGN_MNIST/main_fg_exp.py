...
def main(config):
    ...
    if config.mode == 'train':
        train_loader = get_loader(config.image_dir, config.z_path, config.label_path, 
                            'train', config.image_size, config.batch_size, 0)
        test_loader = get_loader(config.image_dir, config.z_path, config.label_path, 
                            'test', config.image_size, config.batch_size, 0)
...
if __name__ == '__main__':
    ...
    # Model configuration.
    parser.add_argument('--z_dim', type=int, default=42, help='dimension of noise')
    parser.add_argument('--c_dim', type=int, default=42, help='dimension of class embedding')
    ...
    #will have to modify if not good results 
    ...
    #saver and loader from checkpoint 
    ...
    # Directories.
    parser.add_argument('--real_dir', type=str, default='') # /media/tera/ILSVRC2012/train_128
    parser.add_argument('--image_dir', type=str, default='/home/lcur1547/dl_code/Repo/input_New/Imgs_CF_new/foreground')
    parser.add_argument('--z_path', type=str, default='/home/lcur1547/dl_code/Repo/input_New/Imgs_CF_new/labels_new_mod2.npy')
    parser.add_argument('--label_path', type=str, default='')
    parser.add_argument('--save_dir', type=str, default='./test/gans_fg_kl_term_temp_l1_both')
    ...
    config.model_save_dir = os.path.join(config.save_dir, 'models/fg')
    config.sample_dir = os.path.join(config.save_dir, 'samples/fg')
