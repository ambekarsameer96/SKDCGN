...
if __name__ == '__main__':
    ...
    images = []
    onlyfiles = [f for f in listdir('imagenet/data/100K_images/ims/') if
                 isfile(join('imagenet/data/100K_images/ims/', f))]
    for f in onlyfiles:
        images.append(np.array(Image.open('imagenet/data/100K_images/ims/' + f)))
    images = np.transpose(np.array(images), (0, 3, 1, 2))

    print('Starting...')
    for i in range(10, 31, 5):
        print(get_inception_score(images, splits=i))
