...
def run(config):
    ...
    images_holder = tf.compat.v1.placeholder(tf.float32, [None, 128, 128, 3])
    activations = inception_activations(images_holder)
    real_acts = tf.compat.v1.placeholder(tf.float32, [None, ACTIVATION_DIM], name = 'real_activations')
    fake_acts = tf.compat.v1.placeholder(tf.float32, [None, ACTIVATION_DIM], name = 'fake_activations')
