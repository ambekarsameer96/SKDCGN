...
#        beta = self.beta(y).unsqueeze(-1).unsqueeze(-1)
        ...
        print("Shape of bn", self.bn(h).shape)
...
class GBlock(nn.Module):
    ...
    def _upsample(self, x):
        print("Shape of x in upsample", x)
        h, w = x.size()[2:]
...
class Encoder(nn.Module):
    def __init__(self, image_size=256, conv_dim=32, z_dim=128, c_dim=128, repeat_num=5):
...
class Discriminator(nn.Module):
    def __init__(self, image_size=256, conv_dim=64, repeat_num=5):
