...
def fit(cfg, cgn, discriminator, dataloader, opts, losses, device):
    ...
    for epoch in pbar:
        ...
        for i, data in enumerate(dataloader):
            ...
            #ssim loss
            _x_gen = Variable( x_gen,  requires_grad=False)
            _x_gt =  Variable( x_gt,  requires_grad=True)
            #print('x_gen', x_gen.shape)
            #print('x_gt', x_gt.shape)
            
            ssim_loss = SSIM(data_range=1., channel=3)
            _ssim_loss = 1-ssim_loss(_x_gen, _x_gt)
            _ssim_loss.backward()
            ssim_value = ssim(_x_gen, _x_gt).item()
            ssim_tot += ssim_value
            #print(ssim_value)
            ...
            # Logging
            if cfg.LOG.LOSSES:
                #mean of ssim values averaged over the batch size
