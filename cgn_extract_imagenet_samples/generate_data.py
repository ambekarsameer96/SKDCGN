...
from cgn_extract_imagenet_samples.models import CGN
...
def interp_sheet(cgn, mode, ys, y_interp, num_midpoints, save_path,
    ...
    if save_noise:
        ...
        torch.save(us, 'cgn_extract_imagenet_samples/data/u_fixed.pth')
    else:
        print('Loading noise vector from disk.')
        us = torch.load('cgn_extract_imagenet_samples/data/u_fixed.pth')
...
# (Yes, I know all cgn_extract_imagenet_samples classes very well by now)
...
def main(args):
    ...
    data_path = join('cgn_extract_imagenet_samples', 'data', time_str + trunc_str)
    ...
    noise_path = join(data_path, 'noise.csv')
    # generate data
    with torch.no_grad():
        for i in trange(666,1000):
            print(i)
            # sample class vector and set up the save path
            # ys = sample_classes(args.mode, args.classes)
            ys = [i, i, i]
            im_name = f'{args.run_name}_'
            ...
            if args.interp:
                ...
                for img_num in range(1000):
                    noise, x_gt, mask, premask, foreground, background, bg_mask = cgn(ys=ys)
                    save_image(mask, join(ims_path, im_name + f'{i}_{img_num}_shape.jpg'))
                    save_image(foreground, join(ims_path, im_name + f'{i}_{img_num}_fg.jpg'))
                    save_image(background, join(ims_path, im_name + f'{i}_{img_num}_bg.jpg'))
                    df_noise = pd.DataFrame(columns=[im_name + f'{i}_{img_num}'] + list(noise))
