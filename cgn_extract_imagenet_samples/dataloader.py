...
class ImagenetVanilla(Dataset) :
    ...
    def __init__(self, train=True):
        ...
        root = join('.', 'cgn_extract_imagenet_samples', 'data')
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])

        # Transforms
        if train:
            ims_path = join(root, 'cgn_extract_imagenet_samples', 'train')
            t_list = [transforms.RandomResizedCrop(224), transforms.RandomHorizontalFlip()]
        else:
            ims_path = join(root, 'cgn_extract_imagenet_samples', 'val')
...
class ImagenetCounterfactual(Dataset):
    ...
    args:
        - ims_path: the relative path to the saved dataset, e.g.,
          "cgn_extract_imagenet_samples/data/2021_01_25_16_counterfactuals_trunc_0.5"
...
class CueConflict(Dataset):
    def __init__(self, t_list=[transforms.Resize(256),
        ...
        path = 'cgn_extract_imagenet_samples/data/cue_conflict/'
...
def get_in9_dls(distributed, batch_size, workers, variations=['mixed_rand', 'mixed_same']):
    dls_in9 = {}
    for v in variations:
        in9_ds = Imagenet9(join('.', 'cgn_extract_imagenet_samples', 'data', 'in9', v))
