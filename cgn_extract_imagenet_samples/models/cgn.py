...
from cgn_extract_imagenet_samples.models import BigGAN, U2NET
...
class CGN(nn.Module):
    def __init__(self, batch_sz, truncation=0.5, pretrained=True):
        ...
        # pretrained weights
        biggan_weights = 'cgn_extract_imagenet_samples/weights/biggan256.pth' if pretrained else None
        u2net_weights = 'cgn_extract_imagenet_samples/weights/u2net.pth' if pretrained else None
    ...
    def forward(self, inp=None, ys=None):
        ...
        return inp0[0].cpu().detach().numpy(), x_gt, mask, premask.detach(), foreground, background, background_mask
