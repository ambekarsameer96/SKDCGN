...
def main(config):
    ...
    if config.mode == 'train':
        train_loader = get_loader(config.image_dir, config.z_path, config.label_path, 
                            'train', config.image_size, config.batch_size, config.num_workers, config.im)
        test_loader = get_loader(config.image_dir, config.z_path, config.label_path, 
                            'test', config.image_size, config.batch_size, config.num_workers, config.im)
        ...
        if config.mode == 'test_intra_quick':
            ...
            if config.mode == 'test_big':
                ...
                solver.test_loader = get_loader(config.image_dir, config.z_path, config.label_path, 
                                    'test', config.image_size, 1, config.num_workers, isInter=True, im=config.im)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--im', type=str, default='shape', help='The IM you want to train. Choices are shape, bg, and fg')
    # Model configuration.
    ...
    parser.add_argument('--save_dir', type=str, default='gans')
    ...
    config.model_save_dir = os.path.join(config.save_dir, f'models/{config.im}')
    config.sample_dir = os.path.join(config.save_dir, f'samples/{config.im}')
    config.result_dir = os.path.join(config.save_dir, f'results/{config.im}')
    print(config)
    main(config)
