...
class My_Dataset(data.Dataset):
    def __init__(self, image_dir, z_path, label_path, mode, im, transform):
    ...
    def preprocess(self, num_test=100000):
        """ Preprocess the label file. """
        self.dataset = []
        #        selec_labels =  set(np.load('bad_ids_dw.npy'))  #set(np.load('hier_label.npy'))
        for img_dir, z_path, label_path in zip(self.image_dir.split('+'), self.z_path.split('+'),
                                               self.label_path.split('+')):
            filenames = os.listdir(img_dir)
            noises = np.load(z_path, allow_pickle=True)
            labels = []

            for i, fn in enumerate(filenames):
                if self.im not in fn: continue

                arr = fn.split('_')
                cls, num = int(arr[4]), int(arr[5])
                idx = cls * 1000 + num

                labels.append(cls)
                #                if labels[idx] not in selec_labels: continue
                self.dataset.append([os.path.join(img_dir, fn), noises[idx], cls])
...
class Real_Dataset(data.Dataset):
    ...
    def preprocess(self):
        ...
        for dir_name in dir_names:
            ...
            filenames = glob(os.path.join(self.real_dir, dir_name, '*_fg.JPG'))
            for fn in filenames:
...
def get_loader(image_dir, z_path, label_path, mode, image_size=128, batch_size=16, num_workers=4, isReal=False,
               isInter=False, im='shape'):
    ...
    if isReal:
        ...
        dataset = My_Dataset(image_dir, z_path, label_path, mode, im, transform=transform)
